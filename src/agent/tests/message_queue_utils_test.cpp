...
const nlohmann::json BASE_DATA_CONTENT =
    R"([{"data": {"id":"112233", "args": ["origin_test",
...
public:
  MOCK_METHOD(boost::asio::awaitable<Message>, getNextNAwaitable,
              ...
              (override));
  MOCK_METHOD(int, popN, (MessageType, int, const std::string module),
              (override));
  MOCK_METHOD(int, push, (Message, bool), (override));
  MOCK_METHOD(int, push, (std::vector<Message>), (override));
  MOCK_METHOD(bool, isEmpty, (MessageType, const std::string moduleName),
              (override));
  MOCK_METHOD(Message, getNext,
              (MessageType, const std::string moduleName,
               const std::string moduleType),
              (override));
};
...
protected:
  MessageQueueUtilsTest() : mockQueue(std::make_shared<MockMultiTypeQueue>()) {}
...
TEST_F(MessageQueueUtilsTest, GetMessagesFromQueueTest) {
  ...
  EXPECT_CALL(*mockQueue, getNextNAwaitable(MessageType::STATEFUL, 1, "", ""))
      .WillOnce([&testMessage]() -> boost::asio::awaitable<Message> {
        co_return testMessage;
      });
  ...
  ASSERT_TRUE(result.wait_for(std::chrono::milliseconds(1)) ==
...
TEST_F(MessageQueueUtilsTest, PushCommandsToQueueTest) {
  ...
  EXPECT_CALL(*mockQueue, push(::testing::ContainerEq(expectedMessages)))
      .Times(1);
...
TEST_F(MessageQueueUtilsTest, GetCommandFromQueueEmptyTest) {
  EXPECT_CALL(*mockQueue, isEmpty(MessageType::COMMAND, ""))
...
TEST_F(MessageQueueUtilsTest, GetCommandFromQueueTest) {
  ...
  EXPECT_CALL(*mockQueue, getNext(MessageType::COMMAND, "", ""))
  ...
  ASSERT_EQ(cmd.has_value() ? cmd.value().Parameters
                            : nlohmann::json::array({""}),
            nlohmann::json::array({"parameters_test"}));
  ASSERT_EQ(cmd.has_value() ? cmd.value().ExecutionResult.ErrorCode

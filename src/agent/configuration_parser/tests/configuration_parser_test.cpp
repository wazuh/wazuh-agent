...
TEST(ConfigurationParser, GetConfigBytes)
{
    ...
    const auto parserStr = std::make_unique<configuration::ConfigurationParser>(strConfig);
    const auto ret = parserStr->GetConfig<std::size_t>("agent_array", "size_bytes").value_or(1234);
    ASSERT_EQ(ret, 500);
    const auto retKB = parserStr->GetConfig<std::size_t>("agent_array", "size_KB").value_or(1234);
    ASSERT_EQ(retKB, 45000);
    const auto retMB = parserStr->GetConfig<std::size_t>("agent_array", "size_MB").value_or(1234);
    ASSERT_EQ(retMB, 1000000);
    const auto retM = parserStr->GetConfig<std::size_t>("agent_array", "size_M").value_or(1234);
    ASSERT_EQ(retM, 4000000);
    const auto retGB = parserStr->GetConfig<std::size_t>("agent_array", "size_GB").value_or(1234);

...
int ReadExecConfig()
{
    ...
    if (!fp) {
        LogError(FOPEN_ERROR, DEFAULTAR, errno, strerror(errno));
        return (0);
    }
    ...
    while (fgets(buffer, OS_MAXSTR, fp) != NULL) {
        ...
        if (buffer[0] == '!') {
            LogError(EXEC_INV_CONF, DEFAULTAR);
            continue;
        }
        ...
        *tmp_str = '\0';
        ...
        if (bytes_written < 0) {
            LogError(EXEC_BAD_NAME " Error %d (%s).", exec_names[exec_size], errno, strerror(errno));
        } else if ((size_t)bytes_written >= sizeof(exec_names[exec_size])) {
            LogError(EXEC_BAD_NAME, exec_names[exec_size]);
        }
        ...
        *tmp_str = '\0';
        ...
        if (w_ref_parent_folder(str_pt)) {
            LogError("Active response command '%s' vulnerable to directory traversal attack. Ignoring.", str_pt);
            ...
            if (!process_file) {
                if (f_time_reading) {
                    LogInfo("Active response command not present: '%s'. "
...
char *GetCommandbyName(const char *name, int *timeout)
{
    ...
    if (name[0] == '!') {
        if (w_ref_parent_folder(name + 1)) {
            LogWarn("Active response command '%s' vulnerable to directory traversal attack. Ignoring.", name + 1);
            return NULL;
        }
        ...
        if (snprintf(command, sizeof(command), "%s/%s", AR_BINDIR, name + 1) >= (int)sizeof(command)) {
            LogWarn("Cannot execute command '%32s...': path too long.", name + 1);
            return NULL;
        }
...
void ExecCmd(char *const *cmd)
{
    ...
    if (pid == 0) {
        if (execv(*cmd, cmd) < 0) {
            LogError(EXEC_CMDERROR, *cmd, strerror(errno));
            exit(1);
        }
...
void ExecCmd_Win32(char *cmd)
{
    ...
    if (!CreateProcess(NULL, cmd, NULL, NULL, FALSE, 0, NULL, NULL,
                       &si, &pi)) {
        LogError("Unable to create active response process. ");
        return;
    }

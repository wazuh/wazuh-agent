    ...
    for (i = 0; rootcheck.ignore[i] != NULL; i++) {
        if (rootcheck.ignore_sregex[i]) {
            if (OSMatch_Execute(path_to_ignore, strlen(path_to_ignore), rootcheck.ignore_sregex[i])) {
                LogDebug("'%s' matches the '%s' pattern, so it will be ignored.", path_to_ignore, rootcheck.ignore_sregex[i]->raw);
                return 1;
            }
#ifndef WIN32
...
#endif
            LogDebug("'%s' has been marked as ignored.", path_to_ignore);
            return 1;
        }
    }

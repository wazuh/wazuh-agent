...
int set_winsacl(const char *dir, directory_t *configuration) {
    ...
    LogDebug(FIM_SACL_CONFIGURE, dir);

    if (!OpenProcessToken(GetCurrentProcess(), TOKEN_ADJUST_PRIVILEGES, &hdle)) {
        LogError(FIM_ERROR_SACL_OPENPROCESSTOKEN, GetLastError());
        return 1;
    }
    ...
    if (result = GetNamedSecurityInfo(dir, SE_FILE_OBJECT, SACL_SECURITY_INFORMATION, NULL, NULL, NULL, &old_sacl, &security_descriptor), result != ERROR_SUCCESS) {
        LogError(FIM_ERROR_SACL_GETSECURITYINFO, result);
        goto end;
    }
    ...
    case 0:
        ...
        goto end;
    case 1:
        LogDebug(FIM_SACL_CHECK_CONFIGURE, dir);
        ...
        if (!old_sacl) {
            ...
            if (!GetAclInformation(old_sacl, (LPVOID)&old_sacl_info, sizeof(ACL_SIZE_INFORMATION), AclSizeInformation)) {
                LogError(FIM_ERROR_SACL_GETSIZE, dir);
                goto end;
            }
        }
        break;
    ...
    if (new_sacl = (PACL)win_alloc(new_sacl_size), !new_sacl) {
        LogError(FIM_ERROR_SACL_NOMEMORY, dir);
        goto end;
    }

    if (!InitializeAcl(new_sacl, new_sacl_size, ACL_REVISION)) {
        LogError(FIM_ERROR_SACL_CREATE, dir);
        goto end;
    }
    ...
    if (ace = (SYSTEM_AUDIT_ACE *)win_alloc(sizeof(SYSTEM_AUDIT_ACE) + ev_sid_size - sizeof(DWORD)), !ace) {
        LogError(FIM_ERROR_SACL_ACE_NOMEMORY, dir);
        goto end;
    }
    ...
    if (!AddAce(new_sacl, ACL_REVISION, 0, (LPVOID)ace, ace->Header.AceSize)) {
        LogError(FIM_ERROR_SACL_ACE_ADD, dir);
        goto end;
    }
    ...
    if (result = SetNamedSecurityInfo((char *) dir, SE_FILE_OBJECT, SACL_SECURITY_INFORMATION, NULL, NULL, NULL, new_sacl), result != ERROR_SUCCESS) {
        LogError(FIM_ERROR_SACL_SETSECURITYINFO, result);
        goto end;
    }
    ...
    if (privilege_enabled) {
        ...
        if (set_privilege(hdle, priv, FALSE)) {
            LogError(FIM_ERROR_SACL_SET_PRIVILEGE, GetLastError());
        }
    }
...
int is_valid_sacl(PACL sacl, int is_file) {
    ...
    if (!everyone_sid) {
        if (!AllocateAndInitializeSid(&world_auth, 1, SECURITY_WORLD_RID, 0, 0, 0, 0, 0, 0, 0, &everyone_sid)) {
            LogError(FIM_ERROR_WHODATA_GET_SID, GetLastError());
            return 2;
        }
    }

    if (!sacl) {
        LogDebug(FIM_SACL_NOT_FOUND);
        return 1;
    }

    for (i = 0; i < sacl->AceCount; i++) {
        if (!GetAce(sacl, i, (LPVOID*)&ace)) {
            LogError(FIM_ERROR_WHODATA_GET_ACE, GetLastError());
            return 1;
        }
...
int set_privilege(HANDLE hdle, LPCTSTR privilege, int enable) {
    ...
    if (!LookupPrivilegeValue(NULL, privilege, &pr_uid)) {
        LogError(FIM_ERROR_SACL_FIND_PRIVILEGE, privilege, GetLastError());
        return 1;
    }
    ...
    if (!AdjustTokenPrivileges(hdle, 0, &tp, sizeof(TOKEN_PRIVILEGES), (PTOKEN_PRIVILEGES)NULL, (PDWORD)NULL)) {
        LogError(FIM_ERROR_WHODATA_TOKENPRIVILEGES, GetLastError());
        return 1;
    }

    if (enable) {
        LogDebug(FIM_ELEVATE_PRIVILEGE, privilege);
    } else {
...
int run_whodata_scan() {
    ...
    if (evt_subscribe_handle == NULL) {
        LogError(FIM_ERROR_WHODATA_EVENTCHANNEL);
        return 1;
    }

    LogInfo(FIM_WHODATA_STARTED);
    ...
    OSList_foreach(node_it, syscheck.directories) {
        ...
        if (dir_it->dirs_status.status & WD_IGNORE_REST) {
            ...
            if (!DeleteAce(sacl_it, 0)) {
                LogError(FIM_ERROR_SACL_ACE_DELETE, GetLastError());
                break;
            }
            ...
            LogDebug(FIM_SACL_RESTORED, dir_it->path);
        }
    }
    w_rwlock_unlock(&syscheck.directories_lock);
...
int restore_audit_policies() {
    ...
    if (IsFile(WPOL_BACKUP_FILE)) {
        LogError(FIM_ERROR_WHODATA_RESTORE_POLICIES);
        return 1;
    }
    ...
    do {
        ...
        if (wm_exec_ret_code < 0) {
            LogError(FIM_ERROR_WHODATA_AUDITPOL, "failed to execute command");
            cmd_failed = 1;
        } else if (wm_exec_ret_code == 1) {
            LogError(FIM_ERROR_WHODATA_AUDITPOL, "time overtaken while running the command");
            os_free(cmd_output);
            cmd_failed = 1;
        ...
        if (cmd_failed) {
            LogError(FIM_AUDITPOL_ATTEMPT_FAIL, i+1);
        }
        i++;
...
PEVT_VARIANT whodata_event_render(EVT_HANDLE event) {
    ...
    if (!EvtRender(context, event, EvtRenderEventValues, used_size, buffer, &used_size, &property_count)) {
        LogWarn(FIM_WHODATA_RENDER_EVENT, GetLastError());
        os_free(buffer);
        return buffer;
    }

    if (property_count != fields_number) {
        LogWarn(FIM_WHODATA_RENDER_PARAM);
        os_free(buffer);
    }
...
int whodata_get_event_id(const PEVT_VARIANT raw_data, short *event_id) {
    ...
    if (raw_data[RENDERED_EVENT_ID].Type != EvtVarTypeUInt16) {
        LogWarn(FIM_WHODATA_PARAMETER, raw_data[RENDERED_EVENT_ID].Type, "event_id");
        return -1;
    }
    ...
    if (raw_data[RENDERED_HANDLE_ID].Type != EvtVarTypeHexInt64) {
        if (raw_data[RENDERED_HANDLE_ID].Type == EvtVarTypeSizeT) {
            ...
            *handle_id = (unsigned __int64) raw_data[RENDERED_HANDLE_ID].UInt32Val;
            ...
            LogWarn(FIM_WHODATA_PARAMETER, raw_data[RENDERED_HANDLE_ID].Type, "handle_id");
            return -1;
        }
    } else {
...
int whodata_get_access_mask(const PEVT_VARIANT raw_data, unsigned long *mask) {
    ...
    if (raw_data[RENDERED_ACCESS_MASK].Type != EvtVarTypeHexInt32) {
        LogWarn(FIM_WHODATA_PARAMETER, raw_data[RENDERED_ACCESS_MASK].Type, "mask");
        return -1;
    }
...
int whodata_event_parse(const PEVT_VARIANT raw_data, whodata_evt *event_data) {
    ...
    if (raw_data[RENDERED_PATH].Type != EvtVarTypeString) {
        LogWarn(FIM_WHODATA_PARAMETER, raw_data[RENDERED_PATH].Type, "path");
        return -1;
    }  else {
    ...
    if (raw_data[RENDERED_USER_NAME].Type != EvtVarTypeString) {
        LogWarn(FIM_WHODATA_PARAMETER, raw_data[RENDERED_USER_NAME].Type, "user_name");
    ...
    if (raw_data[RENDERED_PROCESS_NAME].Type != EvtVarTypeString) {
        LogWarn(FIM_WHODATA_PARAMETER, raw_data[RENDERED_PROCESS_NAME].Type, "process_name");
    ...
    if (raw_data[RENDERED_PROCESS_ID].Type != EvtVarTypeHexInt64) {
        if (raw_data[RENDERED_PROCESS_ID].Type == EvtVarTypeSizeT) {
            ...
            LogWarn(FIM_WHODATA_PARAMETER, raw_data[RENDERED_PROCESS_ID].Type, "process_id");
    ...
    if (raw_data[RENDERED_USER_SID].Type != EvtVarTypeSid) {
        LogWarn(FIM_WHODATA_PARAMETER, raw_data[RENDERED_USER_SID].Type, "user_id");
        ...
        if (event_data->user_name) {
            LogDebug(FIM_WHODATA_INVALID_UID, event_data->user_name);
        } else {
            LogDebug(FIM_WHODATA_INVALID_UNKNOWN_UID);
        }
        return -1;
    }
        ...
        switch(event_id) {
            ...
            case 4656:
                ...
                if (configuration == NULL && !(mask & (FILE_APPEND_DATA | FILE_WRITE_DATA))) {
                    ...
                    LogDebug(FIM_WHODATA_NOT_ACTIVE, w_evt->path);
                    free_whodata_event(w_evt);
                    w_rwlock_unlock(&syscheck.directories_lock);
                    goto clean;
                }
                ...
                if (configuration != NULL) {
                    ...
                    if (!(configuration->dirs_status.status & WD_CHECK_WHODATA) &&
                        ...
                        LogDebug(FIM_WHODATA_CANCELED, w_evt->path);
                        free_whodata_event(w_evt);
                        w_rwlock_unlock(&syscheck.directories_lock);
                        goto clean;
                    }
                    ...
                    if (depth > configuration->recursion_level) {
                        LogDebug(FIM_MAX_RECURSION_LEVEL, depth, configuration->recursion_level, w_evt->path);
                        free_whodata_event(w_evt);
                        w_rwlock_unlock(&syscheck.directories_lock);
                        goto clean;
                    }
                ...
                if (device_type = check_path_type(w_evt->path), device_type == 2) {
                    ...
                    if (mask & DELETE || mask & FILE_APPEND_DATA) {
                        LogDebug(FIM_WHODATA_REMOVE_FOLDEREVENT, w_evt->path);
                        is_directory = 1;
                    }
                }
                ...
                if (result == 1) {
                    ...
                    LogDebug(FIM_WHODATA_HANDLE_UPDATE, hash_id);
                    if (w_evtdup = OSHash_Delete_ex(syscheck.wdata.fd, hash_id), !w_evtdup) {
                        LogError(FIM_ERROR_WHODATA_HANDLER_REMOVE, hash_id);
                        free_whodata_event(w_evt);
                        goto clean;
                    }
                    ...
                    if (result = whodata_hash_add(syscheck.wdata.fd, hash_id, w_evt, "whodata"), result != 2) {
                        if(result == 1){
                            LogError(FIM_ERROR_WHODATA_EVENTADD, "whodata", hash_id); // LCOV_EXCL_LINE
                        }
                        free_whodata_event(w_evt);
                        goto clean;
                    }
            ...
            case 4663:
                ...
                if (buffer[RENDERED_TIMESTAMP].Type != EvtVarTypeFileTime) {
                    LogWarn(FIM_WHODATA_PARAMETER, buffer[RENDERED_TIMESTAMP].Type, "event_time");
                ...
                if (fim_configuration_directory(w_evt->path) == NULL) {
                    LogDebug(FIM_WHODATA_DIRECTORY_DISCARDED, w_evt->path);
                    w_evt->scan_directory = 2;
                    w_rwlock_unlock(&syscheck.directories_lock);
                    break;
                }
                ...
                if (w_dir = OSHash_Get(syscheck.wdata.directories, w_evt->path), w_dir) {
                    ...
                    if ((buffer[RENDERED_TIMESTAMP].FileTimeVal - w_dir->QuadPart) < FILETIME_SECOND) {
                        ...
                        LogDebug(FIM_WHODATA_DIRECTORY_SCANNED, w_evt->path);
                    ...
                    LogDebug(FIM_WHODATA_CHECK_NEW_FILES, w_evt->path);
                } else {
                    w_rwlock_unlock(&syscheck.wdata.directories->mutex);
                ...
                if (w_evt = OSHash_Delete_ex(syscheck.wdata.fd, hash_id), w_evt && w_evt->path) {
                    ...
                    if (!w_evt->scan_directory) {
                        ...
                        if (w_evt->mask & DELETE) {
                            ...
                            LogDebug(FIM_WHODATA_NO_NEW_FILES, w_evt->path, w_evt->mask);
                        }

                    } else if (w_evt->scan_directory == 2) {
                        LogDebug(FIM_WHODATA_SCAN_ABORTED, w_evt->path);
                    }
                }
            ...
            case 4719:
                if (policies_checked) {
                    LogWarn(FIM_WHODATA_POLICY_CHANGE_CHANNEL);
                    win_whodata_release_resources(&syscheck.wdata);
                }
            break;
            default:
                LogError(FIM_ERROR_WHODATA_EVENTID);
                goto clean;
        }
    }
    retval = 0;
    ...
    if(!Status) {
        ...
        if(!Status) {
            if(AuditEvents->AuditingMode) {
                ...
                LogDebug(FIM_WHODATA_POLICY_OPENED);
                ...
                while (policyAuditEventType < AuditEvents->MaximumAuditEventCount) {
                    ...
                    if(!strcasecmp(guid_string, guid_ObjectAccess)) {
                        LogDebug(FIM_WHODATA_OBJECT_ACCESS, guid_string);
                        ...
                        for(ULONG subcategoryIndex = 0; subcategoryIndex < subCategoryCount; subcategoryIndex++) {
                            ...
                            if(currentPolicy.AuditingInformation & POLICY_AUDIT_EVENT_SUCCESS) {
                                if(!strcasecmp(guid_string, guid_FileSystem)) {
                                    ...
                                    LogDebug(FIM_WHODATA_SUCCESS_POLICY, "File System", guid_string);
                                }
                                if(!strcasecmp(guid_string, guid_Handle)) {
                                    handle_manipulation_success = 1;
                                    LogDebug(FIM_WHODATA_SUCCESS_POLICY, "Handle Manipulation", guid_string);
                                }
...
error:
    ...
    LogWarn(FIM_WHODATA_ERROR_CHECKING_POL, err_msg, err_code);
        ...
        OSList_foreach(node_it, syscheck.directories) {
            ...
            if (exists) {
                if (!(d_status->status & WD_STATUS_EXISTS)) {
                    LogInfo(FIM_WHODATA_READDED, dir_it->path);
                    if (set_winsacl(dir_it->path, dir_it)) {
                        LogError(FIM_ERROR_WHODATA_ADD_DIRECTORY, dir_it->path);
                    ...
                    if (check_object_sacl(dir_it->path, (d_status->object_type == WD_STATUS_FILE_TYPE)) == 1) {
                        LogInfo(FIM_WHODATA_SACL_CHANGED, dir_it->path);
                ...
                LogDebug(FIM_WHODATA_DELETE, dir_it->path);
                d_status->status &= ~WD_STATUS_EXISTS;
...
int set_policies() {
    ...
    if (!IsFile(WPOL_BACKUP_FILE) && remove(WPOL_BACKUP_FILE)) {
        LogError(FIM_ERROR_WPOL_BACKUP_FILE_REMOVE, WPOL_BACKUP_FILE, strerror(errno), errno);
        goto end;
    }
    ...
    if (retval == 2) {
        LogError(FIM_WARN_WHODATA_AUTOCONF);
        goto end;
    }

    if (f_backup = wfopen(WPOL_BACKUP_FILE, "r"), !f_backup) {
        LogError(FIM_ERROR_WPOL_BACKUP_FILE_OPEN, WPOL_BACKUP_FILE, strerror(errno), errno);
        goto end;
    }
    if (f_new = wfopen(WPOL_NEW_FILE, "w"), !f_new) {
        LogError(FIM_ERROR_WPOL_BACKUP_FILE_OPEN, WPOL_NEW_FILE, strerror(errno), errno);
        goto end;
    }
...
int whodata_hash_add(OSHash *table, char *id, void *data, char *tag) {
    ...
    if (result = OSHash_Add_ex(table, id, data), result != 2) {
        if (!result) {
            LogError(FIM_ERROR_WHODATA_EVENTADD, tag, id);
        } else if (result == 1) {
            LogDebug(FIM_ERROR_WHODATA_EVENTADD_DUP, tag, id);
        }
    }
...
int get_volume_names() {
    ...
    if (fh == INVALID_HANDLE_VALUE) {
        ...
        LogWarn("FindFirstVolumeW failed (%u)'%s'", win_error, strerror(win_error));
        FindVolumeClose(fh);
        return success;
    }
    ...
    while (1) {
        ...
        if (volume_name[0]     != L'\\' ||
            ...
            volume_name[index] != L'\\')
        {
            LogWarn("Find Volume returned a bad path: %s", convert_volume);
            break;
        }
        ...
        if (char_count == 0) {
            win_error = GetLastError();
            LogWarn("QueryDosDeviceW failed (%u)'%s'", win_error, strerror(win_error));
            break;
        }
        ...
        if (!FindNextVolumeW(fh, volume_name, ARRAYSIZE(volume_name))) {
            ...
            if (win_error != ERROR_NO_MORE_FILES) {
                LogWarn("FindNextVolumeW failed (%u)'%s'", win_error, strerror(win_error));
                break;
            }
...
int get_drive_names(wchar_t *volume_name, char *device) {
    ...
    while (1) {
        ...
        if (retval = GetLastError(), retval != ERROR_MORE_DATA) {
            LogWarn("GetVolumePathNamesForVolumeNameW (%u)'%s'", retval, strerror(retval));
            break;
        }
    ...
    if (success) {
        ...
        for (nameit = names; nameit[0] != L'\0'; nameit += name_len + 1) {
            ...
            LogDebug(FIM_WHODATA_DEVICE_LETTER, device, convert_name);
...
void replace_device_path(char **path) {
    ...
    while (syscheck.wdata.device[iterator]) {
        ...
        LogDebug(FIM_WHODATA_DEVICE_PATH, syscheck.wdata.device[iterator], *path);
        ...
        if (!strncmp(*path, syscheck.wdata.device[iterator], dev_size)) {
            ...
            LogDebug(FIM_WHODATA_DEVICE_REPLACE, *path, new_path);
...
char *get_whodata_path(const short unsigned int *win_path) {
    ...
    if (count <= 0) {
        LogDebug(FIM_WHODATA_PATH_NOPROCCESED, error);
        os_free(path);
    }
...
int whodata_path_filter(char **path) {
    if (check_removed_file(*path)) {
        LogDebug(FIM_DISCARD_RECYCLEBIN, *path);
        return 1;
    }
...
int whodata_check_arch() {
    ...
    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, environment_key, 0, KEY_READ, &RegistryKey) != ERROR_SUCCESS) {
        ...
        if (result = RegQueryValueEx(RegistryKey, TEXT(processor_arch), NULL, NULL, (LPBYTE)&arch, &data_size), result != ERROR_SUCCESS) {
            LogError(FIM_ERROR_WHODATA_WIN_ARCH, (unsigned int)result);
        } else {
...
int w_update_sacl(const char *obj_path) {
    ...
    if (!everyone_sid) {
        if (!AllocateAndInitializeSid(&world_auth, 1, SECURITY_WORLD_RID, 0, 0, 0, 0, 0, 0, 0, &everyone_sid)) {
            LogError(FIM_ERROR_WHODATA_WIN_SIDERROR, GetLastError());
            goto end;
        }
    }
    ...
    if (!OpenProcessToken(GetCurrentProcess(), TOKEN_ADJUST_PRIVILEGES, &hdle)) {
        LogError(FIM_ERROR_WHODATA_OPEN_TOKEN, GetLastError());
        goto end;
    }

    if (set_privilege(hdle, priv, TRUE)) {
        LogError(FIM_ERROR_WHODATA_ACTIVATE_PRIV, GetLastError());
        goto end;
    }
    ...
    if (result = GetNamedSecurityInfo(obj_path, SE_FILE_OBJECT, SACL_SECURITY_INFORMATION, NULL, NULL, NULL, &old_sacl, &security_descriptor), result != ERROR_SUCCESS) {
        LogError(FIM_ERROR_WHODATA_GETNAMEDSECURITY, result);
        goto end;
    }
    ...
    if (old_sacl && !GetAclInformation(old_sacl, (LPVOID)&old_sacl_info, sizeof(ACL_SIZE_INFORMATION), AclSizeInformation)) {
        LogError(FIM_ERROR_WHODATA_SACL_SIZE, obj_path);
        goto end;
    }
    ...
    if (new_sacl = (PACL)win_alloc(new_sacl_size), !new_sacl) {
        LogError(FIM_ERROR_WHODATA_SACL_MEMORY, obj_path);
        goto end;
    }

    if (!InitializeAcl(new_sacl, new_sacl_size, ACL_REVISION)) {
        LogError(FIM_ERROR_WHODATA_SACL_NOCREATE, obj_path, GetLastError());
        goto end;
    }

    if (ace = (SYSTEM_AUDIT_ACE *)win_alloc(sizeof(SYSTEM_AUDIT_ACE) + ev_sid_size - sizeof(DWORD)), !ace) {
        LogError(FIM_ERROR_WHODATA_ACE_MEMORY, obj_path, GetLastError());
        goto end;
    }
    ...
    if (!CopySid(ev_sid_size, &ace->SidStart, everyone_sid)) {
        LogError(FIM_ERROR_WHODATA_COPY_SID, obj_path, ev_sid_size, GetLastError());
        goto end;
    }
    ...
    if (old_sacl) {
        if (old_sacl_info.AceCount) {
            for (i = 0; i < old_sacl_info.AceCount; i++) {
               if (!GetAce(old_sacl, i, &entry_access_it)) {
                   LogError(FIM_ERROR_WHODATA_ACE_NOOBTAIN, i, obj_path);
                   goto end;
               }

               if (!AddAce(new_sacl, ACL_REVISION, MAXDWORD, entry_access_it, ((PACE_HEADER)entry_access_it)->AceSize)) {
                   LogError(FIM_ERROR_WHODATA_ACE_NUMBER, i, obj_path);
                   goto end;
               }
           }
        }
    ...
    // Add the new ACE
    if (!AddAce(new_sacl, ACL_REVISION, 0, (LPVOID)ace, ace->Header.AceSize)) {
        LogError(FIM_ERROR_WHODATA_ACE_NOADDED, obj_path, GetLastError());
        goto end;
    }
    ...
    if (result = SetNamedSecurityInfo((char *) obj_path, SE_FILE_OBJECT, SACL_SECURITY_INFORMATION, NULL, NULL, NULL, new_sacl), result != ERROR_SUCCESS) {
        LogError(FIM_ERROR_WHODATA_SETNAMEDSECURITY, result);
        goto end;
    }

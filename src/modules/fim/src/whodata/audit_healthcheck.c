...
int audit_health_check(int audit_socket) {
    ...
    if (retval <= 0 && retval != -EEXIST) {
        LogDebug(FIM_AUDIT_HEALTHCHECK_RULE);
        return -1;
    }

    LogDebug(FIM_AUDIT_HEALTHCHECK_START);
    ...
    do {
        ...
        if (!fp) {
            LogDebug(FIM_AUDIT_HEALTHCHECK_FILE);
        } else {
            fclose(fp);
        }
    ...
    if (atomic_int_get(&audit_health_check_creation) == 0) {
        ...
        LogDebug(FIM_HEALTHCHECK_CREATE_ERROR);
        retval = -1;
    } else {
        LogDebug(FIM_HEALTHCHECK_SUCCESS);
        retval = 0;
    }
    ...
    if (audit_delete_rule(abs_path_healthcheck, WHODATA_PERMS, AUDIT_HEALTHCHECK_KEY) <= 0) {
        LogDebug(FIM_HEALTHCHECK_CHECK_RULE); // LCOV_EXCL_LINE
    }

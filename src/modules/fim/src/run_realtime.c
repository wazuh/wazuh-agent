...
int realtime_start() {
    ...
    if (syscheck.realtime->fd < 0) {
        LogError(FIM_ERROR_INOTIFY_INITIALIZE);
        goto error;
    }
...
int fim_add_inotify_watch(const char *dir, const directory_t *configuration) {
    ...
    if (syscheck.realtime->fd < 0) {
        ...
        if (wd < 0) {
            if (errno == 28) {
                LogError(FIM_ERROR_INOTIFY_ADD_MAX_REACHED, dir, wd, errno);
            }
            else {
                LogDebug(FIM_INOTIFY_ADD_WATCH, dir, wd, errno, strerror(errno));
            }
        }
        else {
            char wdchar[33];
            ...
            if (!OSHash_Get_ex(syscheck.realtime->dirtb, wdchar)) {
                if (retval = OSHash_Add_ex(syscheck.realtime->dirtb, wdchar, data), retval == 0) {
                    os_free(data);
                    LogCritical(FIM_CRITICAL_ERROR_OUT_MEM);
                }
                else if (retval == 1) {
                    LogDebug(FIM_REALTIME_HASH_DUP, data);
                    os_free(data);
                }

                LogDebug(FIM_REALTIME_NEWDIRECTORY, dir);
            }
            else {
                if (retval = OSHash_Update_ex(syscheck.realtime->dirtb, wdchar, data), retval == 0) {
                    LogError("Unable to update 'dirtb'. Directory not found: '%s'", data);
                    os_free(data);
                    w_mutex_unlock(&syscheck.fim_realtime_mutex);
                    return (-1);
                }
...
void realtime_process() {
    ...
    if (len < 0) {
        LogError(FIM_ERROR_REALTIME_READ_BUFFER);
        return;
    }
    ...
    for (size_t i = 0; i < (size_t) len; i += REALTIME_EVENT_SIZE + event->len) {
        ...
        if (event->wd == -1 && event->mask == IN_Q_OVERFLOW) {
            LogWarn("Real-time inotify kernel queue is full. Some events may be lost. Next scheduled scan will recover lost data.");
        ...
        if (rbtree_insert(tree, final_name, NULL) == NULL) {
            LogDebug("Duplicate event in real-time buffer: %s", final_name);
        }
        ...
        case IN_MOVE_SELF:
            ...
            // fall through
        case IN_DELETE_SELF:
            LogDebug(FIM_INOTIFY_WATCH_DELETED, entry);
...
int realtime_update_watch(const char *wd, const char *dir) {
    ...
    if (new_wd < 0) {
        if (errno == ENOSPC) {
            LogError(FIM_ERROR_INOTIFY_ADD_MAX_REACHED, dir, new_wd, errno);
            return -1;
        } else if (errno == ENOENT) {
            LogDebug("Removing watch on non existent directory '%s'", dir);
            inotify_rm_watch(syscheck.realtime->fd, old_wd);
            free(OSHash_Delete_ex(syscheck.realtime->dirtb, wd));
            return 0;
        } else {
            LogDebug(FIM_INOTIFY_ADD_WATCH, dir, new_wd, errno, strerror(errno));
            return -1;
        }
    }
    ...
    if (!OSHash_Get_ex(syscheck.realtime->dirtb, wdchar)) {
        ...
        LogDebug(FIM_REALTIME_NEWDIRECTORY, data);
    ...
    if(syscheck.realtime->fd) {
        ...
        while(hash_node) {
            ...
            if (strncmp(dir_slash, data, strlen(dir_slash)) == 0) {
                ...
                LogDebug(FIM_INOTIFY_WATCH_DELETED, data);
                os_free(data_node);
...
void realtime_sanitize_watch_map() {
    ...
    gettime(&end);
    LogDebug("Time spent sanitizing wd hashmap: %.3f seconds", time_diff(&start, &end));
...
void CALLBACK RTCallBack(DWORD dwerror, DWORD dwBytes, LPOVERLAPPED overlap)
{
    ...
    if (dwerror != ERROR_SUCCESS) {
        ...
        if (messageBuffer) {
            ...
            LogError(FIM_ERROR_REALTIME_WINDOWS_CALLBACK, messageBuffer, dwerror);
    ...
    if (rtlocald == NULL) {
        LogError(FIM_ERROR_REALTIME_WINDOWS_CALLBACK_EMPTY);
        w_mutex_unlock(&syscheck.fim_realtime_mutex);
        w_rwlock_unlock(&syscheck.directories_lock);
        return;
    }
    ...
    if(rtlocald->watch_status == FIM_RT_HANDLE_CLOSED) {
        ...
        LogDebug(FIM_REALTIME_CALLBACK, wdchar);
        w_mutex_unlock(&syscheck.fim_realtime_mutex);
        w_rwlock_unlock(&syscheck.directories_lock);
        return;
    }
    ...
    if (dwBytes) {
        ...
        LogWarn(FIM_WARN_REALTIME_OVERFLOW);
    }
...
int realtime_adddir(const char *dir, directory_t *configuration) {
    ...
    if (FIM_MODE(configuration->options) == FIM_WHODATA) {
        ...
        if (!syscheck.wdata.fd && whodata_audit_start()) {
            LogCritical(FIM_CRITICAL_DATA_CREATE, "whodata file descriptors");
        }
        ...
        if (type = check_path_type(dir), type == 2) {
            ...
            LogDebug(FIM_WARN_REALTIME_OPENFAIL, dir);
        ...
        if (set_winsacl(dir, configuration)) {
            LogError(FIM_ERROR_WHODATA_ADD_DIRECTORY, dir);
            return -2;
        }
    ...
    if(rtlocald != NULL) {
        if (!w_directory_exists(rtlocald->dir)) {
            if (rtlocald->watch_status == FIM_RT_HANDLE_CLOSED) {
                LogDebug(FIM_REALTIME_CALLBACK, rtlocald->dir);
    ...
    if (_get_realtime_watches() >= syscheck.max_fd_win_rt) {
        LogDebug(FIM_REALTIME_MAXNUM_WATCHES, dir);
        w_mutex_unlock(&syscheck.fim_realtime_mutex);
        return 0;
    }
...
int realtime_start() {
    LogError(FIM_ERROR_REALTIME_INITIALIZE);
...
void fim_realtime_print_watches() {
    w_mutex_lock(&syscheck.fim_realtime_mutex);
    if (syscheck.realtime != NULL) {
        LogDebug(FIM_NUM_WATCHES, OSHash_Get_Elem_ex(syscheck.realtime->dirtb));
    }
    w_mutex_unlock(&syscheck.fim_realtime_mutex);
}

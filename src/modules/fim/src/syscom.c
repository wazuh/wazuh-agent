...
error:
    LogDebug(FIM_SYSCOM_FAIL_GETCONFIG, section);
...
size_t syscom_dispatch(char * command, char ** output){
    ...
    if (strncmp(command, HC_FIM_FILE, strlen(HC_FIM_FILE)) == 0
        ...
        if (strcmp(rcv_comm, "getconfig") == 0){
            // getconfig section
            if (!rcv_args){
                LogDebug(FIM_SYSCOM_ARGUMENTS, "getconfig");
    ...
    LogDebug(FIM_SYSCOM_UNRECOGNIZED_COMMAND, command);
...
void * syscom_main(__attribute__((unused)) void * arg) {
    ...
    LogDebug(FIM_SYSCOM_REQUEST_READY);

    if (sock = OS_BindUnixDomain(SYS_LOCAL_SOCK, SOCK_STREAM, OS_MAXSTR), sock < 0) {
        LogError(FIM_ERROR_SYSCOM_BIND_SOCKET, SYS_LOCAL_SOCK, errno, strerror(errno));
        return NULL;
    }
    ...
    while (1) {
        ...
        case -1:
            if (errno != EINTR) {
                LogCritical(FIM_CRITICAL_ERROR_SELECT, "syscom_main()", strerror(errno));
            }
        ...
        if (peer = accept(sock, NULL, NULL), peer < 0) {
            if (errno != EINTR) {
                LogError(FIM_ERROR_SYSCOM_ACCEPT, strerror(errno));
            }
        ...
        case OS_SOCKTERR:
            LogError(FIM_ERROR_SYSCOM_RECV_TOOLONG);
            break;

        case -1:
            LogError(FIM_ERROR_SYSCOM_RECV, strerror(errno));
            break;

        case 0:
            LogDebug(FIM_SYSCOM_EMPTY_MESSAGE);
            close(peer);
            break;

        case OS_MAXLEN:
            LogError(FIM_ERROR_SYSCOM_RECV_MAXLEN, MAX_DYN_STR);
            close(peer);
            break;
    ...
    LogDebug(FIM_SYSCOM_THREAD_FINISED);

...
bool w_journald_can_read(unsigned long owner_id) {
    ...
    if (gs_journald_global.owner_id == 0) {
        ...
        if (gs_journald_global.journal_ctx == NULL && w_journal_context_create(&gs_journald_global.journal_ctx) != 0) {
            LogError(LOGCOLLECTOR_JOURNAL_LOG_DISABLING);
        ...
        if (ret < 0) {
            LogError(LOGCOLLECTOR_JOURNAL_LOG_FAIL_SEEK, strerror(-ret));
            gs_journald_global.is_disabled = true;
            return false;
        }

        LogInfo(LOGCOLLECTOR_JOURNALD_MONITORING);
    ...
    *rc = 0;
    ...
    while ((maximum_lines == 0 || count_logs < maximum_lines) && can_read()) {
        ...
        if (result_get_next < 0) {
            LogError(LOGCOLLECTOR_JOURNAL_LOG_FAIL_NEXT, strerror(-result_get_next));
            gs_journald_global.is_disabled = true;
            break;
        } else if (result_get_next == 0) {
            LogDebug(LOGCOLLECTOR_JOURNAL_LOG_NO_NEW);
            break;
        }
        ...
        if (entry_str == NULL) {
            LogError(LOGCOLLECTOR_JOURNAL_LOG_FAIL_GET);
            break;
        }
        ...
        if (entry_str_len > MAX_LINE_LEN) {
            LogDebug(LOGCOLLECTOR_JOURNAL_LOG_TRUNCATED);
...
void w_journald_set_status_from_JSON(cJSON * global_json) {
    ...
    LogDebug(LOGCOLLECTOR_JOURNAL_LOG_SET_LAST, timestamp_uint);
}

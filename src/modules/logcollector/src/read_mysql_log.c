    ...
    *rc = 0;
    ...
    while (can_read() && fgets(str, sizeof(str), lf->fp) != NULL && (!maximum_lines || lines < maximum_lines)) {
       ...
       */
        ...
        if (bytes_written < 0) {
            LogError("Error %d (%s) while reading message: '%s' (length = " FTELL_TT "): '%s'...", errno, strerror(errno), lf->file, FTELL_INT64 bytes_written, buffer);
        } else if ((size_t)bytes_written >= sizeof(buffer)) {
            LogError("Message size too big on file '%s' (length = " FTELL_TT "): '%s'...", lf->file, FTELL_INT64 bytes_written, buffer);
        }

        LogDebug("Reading mysql messages: '%s'", buffer);

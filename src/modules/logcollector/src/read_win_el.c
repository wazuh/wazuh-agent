...
void readel(os_el *el, int printit)
{
                ...
                for (nstr = 0; nstr < el->er->NumStrings && sstr; nstr++) {
                    ...
                    if (tmp_str) {
                        ...
                        *tmp_str = '\0';
                        ...
                        LogError("Invalid application string (size+)");
                    }
    ...
    /* Event log was cleared */
        ...
        msg_alert[512] = '\0';
        LogWarn("Event log cleared: '%s'", el->name);
        ...
        if (startEL(el->name, el) < 0) {
            LogError("Unable to reopen event log '%s'", el->name);
        }
    }
    ...
    /* Event log was closed and re-opened */
        ...
        LogDebug("The EventLog service has been restarted. Reconnecting to '%s' channel.", el->name);
        ...
        if (startEL(el->name, el) < 0) {
            ...
            counter = 0;
            LogInfo("'%s' channel has been reconnected succesfully.", el->name);
        }
    }
        ...
        el->er = NULL;
        if (counter == 0) {
            LogWarn("The EventLog service is down. Unable to collect logs from its channels.");
            counter = 1;
        }
    }

    else {
        LogDebug("Error reading event log: %d", id);
    }
}
...
void win_read_vista_sec()
{
    ...
    if (!fp) LogCritical("Unable to read vista security descriptions.");

    /* Creating the hash */
    vista_sec_id_hash = OSHash_Create();
    if (!vista_sec_id_hash) {
        fclose(fp);
        LogCritical("Unable to read vista security descriptions.");
    }
    ...
    while (fgets(buf, OS_MAXSTR, fp) != NULL) {
        ...
        p = strchr(buf, ',');
        if (!p) {
            LogError("Invalid entry on the Vista security description.");
            continue;
        }
...
void win_startel(char *evt_log)
{
    ...
    if (!dll_hash) {
        ...
        if (!dll_hash) {
            LogError("Unable to create DLL hash.");
        }
    }

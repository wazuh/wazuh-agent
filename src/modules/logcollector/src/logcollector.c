...
int check_ignore_and_restrict(OSList * ignore_exp_list, OSList * restrict_exp_list, const char *log_line) {
    ...
    if (ignore_exp_list) {
        OSList_foreach(node_it, ignore_exp_list) {
            ...
            if (w_expression_match(exp_it, log_line, NULL, NULL)) {
                LogDebug(LF_MATCH_REGEX, log_line, "ignore", w_expression_get_regex_pattern(exp_it));
                return true;
            }
        }
    }
    ...
    if (restrict_exp_list) {
        OSList_foreach(node_it, restrict_exp_list) {
            ...
            if (!w_expression_match(exp_it, log_line, NULL, NULL)) {
                LogDebug(LF_MATCH_REGEX, log_line, "restrict", w_expression_get_regex_pattern(exp_it));
                return true;
            }
        }
    }
...
void LogCollectorStart()
{
    ...
    if (!w_sysinfo_init(sysinfo)) {
        LogError(SYSINFO_DYNAMIC_INIT_ERROR);
    }
#endif
    ...
    if (!checkVista()) {
        LogInfo("Windows version is older than 6.0. (%s).", m_uname);
    } else {
        LogInfo("Windows version is 6.0 or newer. (%s).", m_uname);
    }
    ...
    for (i = 0;; i++) {
        ...
        if (!current->file) {
            ...
            LogInfo(READING_EVTLOG, current->file);
...
#else
            LogWarn("eventchannel not available on this version of Windows");
#endif
            ...
            if (current->command) {
                ...
                LogInfo("Monitoring output of command(%d): %s", current->ign, current->command);
                tg = 0;
                if (current->target) {
                ...
                LogError("Missing command argument. Ignoring it.");
            }
            ...
            if (current->command) {
                ...
                LogInfo("Monitoring full output of command(%d): %s", current->ign, current->command);
                tg = 0;
                if (current->target){
                    while (current->target[tg]) {
                        LogDebug("Socket target for '%s' -> %s", current->command, current->target[tg]);
...
#else
            LogInfo(LOGCOLLECTOR_ONLY_MACOS);
#endif
...
#else
            LogInfo(LOGCOLLECTOR_JOURNALD_ONLY_LINUX);
            ...
            if (current->file) {
                LogInfo(READING_FILE, current->file);
            }
                ...
                for (i = 0, j = -1;; i++) {
                    ...
                    if (current->file && current->exists) {
                        if (reload_file(current) == -1) {
                            LogInfo(FORGET_FILE, current->file);
                            ...
                            if (j >= 0) {
                                if (Remove_Localfile(&(globs[j].gfiles), i, 1, 0,&globs[j])) {
                                    LogError(REM_ERROR, current->file);
                                } else {
                                    LogDebug(CURRENT_FILES, current_files, maximum_files);
                                    i--;
                                    continue;
            ...
            for (i = 0, j = -1;; i++) {
                ...
                if (current->fp) {
                    ...
                    if(tf == NULL) {
                        if (errno == ENOENT) {
                            ...
                            LogError(FOPEN_ERROR, current->file, errno, strerror(errno));
                        }
                    }
                        ...
                        LogError(FSTAT_ERROR, current->file, errno, strerror(errno));
                    }
                    else if (fclose(tf) == EOF) {
                        LogError("Closing the temporary file %s did not work (%d): %s", current->file, errno, strerror(errno));
                    }
#else
                    HANDLE h1;
                    ...
                    if (h1 == INVALID_HANDLE_VALUE) {
                        fclose(current->fp);
                        current->fp = NULL;
                        LogInfo(LOGCOLLECTOR_INVALID_HANDLE_VALUE, current->file);
                ...
                if (current->ign >= open_file_attempts) {
                    ...
                    if(!strcmp(current->logformat, "eventchannel")){
                        ...
                        LogInfo(LOGC_FILE_ERROR, current->file);
                    }
...
int update_fname(int i, int j)
{
    ...
    if (ret == 0) {
        LogCritical(PARSE_ERROR, lf->ffile);
    }
...
int handle_file(int i, int j, __attribute__((unused)) int do_fseek, int do_log)
{
    ...
    if (!lf->fp) {
        if (do_log == 1 && lf->exists == 1) {
            LogError(FOPEN_ERROR, lf->file, errno, strerror(errno));
            lf->exists = 0;
        }
        goto error;
    }
    ...
    if (fstat(fd, &stat_fd) == -1) {
        LogError(FSTAT_ERROR, lf->file, errno, strerror(errno));
        fclose(lf->fp);
        lf->fp = NULL;
        goto error;
    }
    ...
    if (!get_fp_file_information(lf->fp, &lpFileInformation)) {
        LogError("Unable to get file information by handle.");
        fclose(lf->fp);
        lf->fp = NULL;
        goto error;
    }
...
void set_read(logreader *current, int i, int j) {
    ...
    if (current->ffile) {
        ...
        if (update_fname(i, j)) {
            ...
            LogCritical(PARSE_ERROR, current->ffile);
        }
    ...
    if (strcmp("ossecalert", current->logformat) == 0) {
        ...
        if (!init_djbmultilog(current)) {
            LogError(INV_MULTILOG, current->file);
            if (current->fp) {
                fclose(current->fp);
                current->fp = NULL;
            }
        ...
        if (current->filter_binary) {
            ...
            if (FileSizeWin(current->file) == 0) {
                ...
                LogDebug("File '%s' is empty. Setting encoding to UCS-2 LE.",current->file);
                return;
            }
        }
    ...
    if (globs) {
        for (j = 0; globs[j].gpath; j++) {
            ...
            while (g.gl_pathv[glob_offset] != NULL) {
                if (current_files >= maximum_files) {
                    LogWarn(FILE_LIMIT, maximum_files);
                    break;
                }

                struct stat statbuf;
                if (lstat(g.gl_pathv[glob_offset], &statbuf) < 0) {
                    LogError("Error on lstat '%s' due to [(%d)-(%s)]", g.gl_pathv[glob_offset], errno, strerror(errno));
                    glob_offset++;
                    continue;
                }
        ...
        for (j = 0; globs[j].gpath; j++) {
            ...
            while (g.gl_pathv[glob_offset] != NULL) {
                ...
                if(found) {
                    ...
                    if (result) {
                        LogCritical(REM_ERROR,g.gl_pathv[glob_offset]);
                    } else {
                        ...
                        if(!file) {
                            ...
                            LogInfo(EXCLUDE_FILE,g.gl_pathv[glob_offset]);
                        }
                ...
                for (file = 0; result[file] != NULL; file++) {
                    ...
                    if (!found) {
                        ...
                        if (!ex_file) {
                            ...
                            LogInfo(NEW_GLOB_FILE, globs[j].gpath, full_path);
                            os_realloc(globs[j].gfiles, (i + 2) * sizeof(logreader), globs[j].gfiles);
                            /* Copy the current item to the end mark as it should be a pattern */
                            memcpy(globs[j].gfiles + i + 1, globs[j].gfiles + i, sizeof(logreader));
                            // Clone the multiline configuration if it exists
    ...
    if (current->file && !current->command) {
        for (r = 0, k = -1;; r++) {
            ...
            if (current != dup && dup->file && !strcmp(current->file, dup->file)) {
                ...
                if (result) {
                    LogCritical(REM_ERROR, current->file);
                } else {
                    LogDebug(CURRENT_FILES, current_files, maximum_files);
                }
                d_control = NEXT_IT;
...
static void set_sockets() {
    ...
    for (i = 0, t = -1;; i++) {
        ...
        for (j = 0; current->target[j]; j++) {
            ...
            if (found != 0) {
                LogCritical("Socket '%s' for '%s' is not defined.", current->target[j], file);
            } else {
        ...
        if (current->out_format) {
            for (j = 0; current->out_format[j]; ++j) {
                if (current->out_format[j]->target) {
                    ...
                    if (!current->target[k]) {
                        LogWarn("Log target '%s' not found for the output format of localfile '%s'.", current->out_format[j]->target, current->file);
                    }
                } else {
...
void w_msg_hash_queues_init(){
    ...
    if(!msg_queues_table){
        LogCritical("Failed to create hash table for queue threads");
    }
...
int w_msg_queue_push(w_msg_queue_t * msg, const char * buffer, char *file, unsigned long size, logtarget * log_target, char queue_mq) {
    ...
    if ((result < 0) && !reported) {
        #ifndef WIN32
            LogWarn("Target '%s' message queue is full (%zu). Log lines may be lost.", log_target->log_socket->name, msg->msg_queue->size);
        #else
            LogWarn("Target '%s' message queue is full (%u). Log lines may be lost.", log_target->log_socket->name, msg->msg_queue->size);
        #endif
            reported = 1;
    }
    ...
    if (result < 0) {
        ...
        free(message->buffer);
        free(message);
        LogDebug("Discarding log line for target '%s'", log_target->log_socket->name);
    }
...
void * w_output_thread(void * args){
    ...
    if (msg_queue = OSHash_Get(msg_queues_table, queue_name), !msg_queue) {
        LogWarn("Could not found the '%s'.", queue_name);
    #ifdef WIN32
        exit(1);
    #else
        return NULL;
    ...
    while(1)
    {
        ...
        if (strcmp(message->log_target->log_socket->name, "agent") == 0) {
            ...
            if (result != 0) {
                if (result != 1) {
                    LogError("Unable to send message to '%s' (wazuh-agentd might be down). Attempting to reconnect.", DEFAULTQUEUE);
                }
                // Retry to connect infinitely.
                logr_queue = StartMQ(DEFAULTQUEUE, WRITE, INFINITE_OPENQ_ATTEMPTS);

                LogInfo("Successfully reconnected to '%s'", DEFAULTQUEUE);
                ...
                if (result = SendMSGtoSCK(logr_queue, message->buffer, message->file, message->queue_mq, message->log_target),
                    ...
                    if (result != 1) {
                        LogError("Unable to send message to '%s' after a successfull reconnection...", DEFAULTQUEUE);
                    }
                    result = 1;
                }
            }
            ...
            if (retries == MAX_RETRIES) {
                LogError(SEND_ERROR, message->log_target->log_socket->location, message->buffer);
            }
        }
        free(message->file);
        free(message->buffer);
...
void * w_input_thread(__attribute__((unused)) void * t_id){
        ...
        for (i = 0, j = -1;; i++) {
                ...
                if (!current->fp) {
                    ...
                    /* Read the journald logs */
                        ...
                        if (w_journald_can_read(thread_id)) {
                            ...
                            LogDebug(LOGCOLLECTOR_JOURNAL_LOG_NOT_OWNER);
                        }
                    }
#endif
            ...
            if (current->filter_binary) {
                ...
                /* If ferror is set */
                    ...
                    LogError(FREAD_ERROR, current->file, errno, strerror(errno));
    #ifndef WIN32
    ...
    #ifndef WIN32
                        LogError(FSEEK_ERROR, current->file, errno, strerror(errno));
    #endif
    ...
    for (i = 0, j = -1;; i++) {
        ...
        if(current->file && !current->command && current->filter_binary) {
            ...
            if(is_ascii_utf8(current->file,MAX_ASCII_LINES,MAX_UTF8_CHARS)) {
                ...
                if (result) {
                    LogCritical(REM_ERROR, file_name);
                } else {
                    LogDebug(NON_TEXT_FILE, file_name);
                    LogDebug(CURRENT_FILES, current_files, maximum_files);
            ...
            if (wildcard) {
                ...
                *wildcard = '\0';
                ...
                if (dir = opendir(global_path), !dir) {
                    LogError("Couldn't open directory '%s' due to: %s", global_path, win_strerror(WSAGetLastError()));
                    os_free(global_path);
                    continue;
                }
                ...
                while (dirent = readdir(dir), dirent) {
                    ...
                    if (is_dir = opendir(full_path), is_dir) {
                        LogDebug("File %s is a directory. Skipping it.", full_path);
                        closedir(is_dir);
                        continue;
                    }
                    ...
                    if(!OS_Regex(regex,dirent->d_name)) {
                        LogDebug("Regex %s doesn't match with file '%s'",regex,dirent->d_name);
                        os_free(regex);
                        continue;
                    }
                    ...
                    if(found) {
                        ...
                        if (result) {
                            LogCritical(REM_ERROR,full_path);
                        } else {
                            ...
                            if(!file) {
                                ...
                                LogInfo(EXCLUDE_FILE,full_path);
                            }

                            LogDebug(EXCLUDE_FILE,full_path);
                            LogDebug(CURRENT_FILES, current_files, maximum_files);
                        }
                    }
                }
...
STATIC void w_initialize_file_status() {
    ...
    if (files_status = OSHash_Create(), files_status == NULL) {
        LogCritical(HCREATE_ERROR, files_status_name);
    }

    if (OSHash_setSize(files_status, LOCALFILES_TABLE_SIZE) == 0) {
        LogCritical(HSETSIZE_ERROR, files_status_name);
    }
    ...
    if (fd = wfopen(LOCALFILE_STATUS, "r"), fd != NULL) {
        ...
        if (fread(str, 1, OS_MAXSTR - 1, fd) < 1) {
            LogError(FREAD_ERROR, LOCALFILE_STATUS, errno, strerror(errno));
            clearerr(fd);
        } else {
        ...
        LogError(FOPEN_ERROR, LOCALFILE_STATUS, errno, strerror(errno));
    }
}
...
STATIC void w_save_file_status() {
    ...
    if (fd = wfopen(LOCALFILE_STATUS, "w"), fd != NULL) {
        if (fwrite(str, 1, size_str, fd) == 0) {
            LogError(FWRITE_ERROR, LOCALFILE_STATUS, errno, strerror(errno));
            clearerr(fd);
        }
        fclose(fd);
    } else {
        LogCritical(FOPEN_ERROR, LOCALFILE_STATUS, errno, strerror(errno));
    }
    ...
    for (int i = 0; i < array_size; i++) {
        ...
        if (OSHash_Update_ex(files_status, path_str, data) != 1) {
            if (OSHash_Add_ex(files_status, path_str, data) != 2) {
                LogError(HADD_ERROR, path_str, files_status_name);
                EVP_MD_CTX_free(context);
                os_free(data);
            }
        }
...
STATIC int w_set_to_last_line_read(logreader * lf) {
    ...
    if (data = (os_file_status_t *)OSHash_Get_ex(files_status, lf->file), data == NULL) {
        ...
        if (w_update_hash_node(lf->file, w_ftell(lf->fp)) == -1) {
            LogError(HUPDATE_ERROR, lf->file, files_status_name);
        }
        return 0;
    }
    ...
    if (OS_SHA1_File_Nbytes(lf->file, &context, output, OS_BINARY, data->offset) < 0) {
        LogError(FAIL_SHA1_GEN, lf->file);
        EVP_MD_CTX_free(context);
        return -1;
    }
...
STATIC int64_t w_set_to_pos(logreader * lf, int64_t pos, int mode) {
    ...
    if (w_fseek(lf->fp, pos, mode) < 0) {
        LogError(FSEEK_ERROR, lf->file, errno, strerror(errno));
        fclose(lf->fp);
        lf->fp = NULL;
        return -1;
    }

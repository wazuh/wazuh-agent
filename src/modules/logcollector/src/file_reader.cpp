...
Awaitable Localfile::run() {
    ...
    while (true) {
        ...
        std::size_t n = co_await file.async_read_some(buffer, use_awaitable);

        if (n == 0) {
            continue;
        }

        data.append(static_cast<const char *>(buffer.data()), buffer.size());

        std::size_t newline_pos = 0;

        while ((newline_pos = data.find('\n')) != std::string::npos) {
            auto line = data.substr(0, newline_pos + 1);

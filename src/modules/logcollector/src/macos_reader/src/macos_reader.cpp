    ...
    Awaitable MacOSReader::Run()
    {
        ...
        while (m_keepRunning.load())
        {
            const auto logEntries =
                m_osLogStoreWrapper->AllEntries(m_lastLogEntryTimeInSecondsSince1970, m_query, m_logLevel);
    ...
    void MacOSReader::SetQuery(const std::string& query, const std::vector<std::string>& logTypes)
    {
        const auto logTypesPredicate = [&logTypes]() -> std::string
        {
            ...
            for (const auto& type : logTypes)
            {
                ...
                {
                    predicates.emplace_back("eventType == activityCreateEvent OR eventType == activityTransitionEvent "

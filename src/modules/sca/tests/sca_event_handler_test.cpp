...
TEST_F(SCAEventHandlerTest, ProcessStateful_ValidInput1)
{
    const nlohmann::json input = {
        ...
        {"policy",
         {{"new",
           {{"id", "pol1"}, {"name", "CIS Ubuntu Benchmark"}, {"rationale", "SMBv1 is outdated and insecure."}}}}},
        {"result", 0}};
    ...
    EXPECT_EQ(event["check"]["title"], "Ensure firewall is active");
...
{
    const nlohmann::json input = {{"check", {{"id", "chk1"}, {"result", "failed"}}},
                                  {"policy", {{"id", "pol1"}, {"description", "Ensure firewall is running"}}},
                                  {"collector", "check"},
                                  {"result", 1}};

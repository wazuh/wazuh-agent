...
#
# Must test explicit 127.0.0.1 or ::1 host rather than localhost because
...
proc available_ports { n { rangeincr 10 } } {
	...
	# In order to manually test the failure mode for running
	# repmgr tests with IPv6 on a host configured to support
	# only IPv4, set ipv6onipv4 to 1 below and run 
	# run_ipv6onipv4.
	set ipv6onipv4 0
	...
	for { set i $baseport } { $i < $baseport + $rangeincr * 100 } \
		...
		# Try one set of contiguous ports.  If we are testing
		# IPv6 tests on a host configured to use IPv4 only, 
		# set things up so we can test for the correct 
		# error message coming from repmgr_start.
		while { [incr numports -1] >= 0 } {
			incr curport
			if { !$ipv6onipv4 } {
				if [catch { socket -server Unused \
				    -myaddr $hoststr $curport } sock] {
...
# This proc is used for manual testing only.  It tests for the 
# expected type of failure when a repmgr test tries to use IPv6
# messaging on a host configured to use IPv4 only.
proc run_ipv6onipv4 { } {
	global test_names

	foreach test $test_names(repmgr_other) {
		# We skip repmgr017 because it resets the global 
		# variable databases_in_memory and doesn't add any
		# testing value.
		if { $test == "repmgr017" } {
			puts "Skipping repmgr017"
			continue
		}
		puts "Running $test"
		catch {$test} msg
		error_check_good right_error [is_substr $msg\
		    "cannot assign requested address"] 1
	}
}

# This proc is used to return the default loopback address, assuming 
# a default of IPv6 but allowing an IPv4 override.
global ipversion
if { [info exists ipversion] == 0 } { 
	set ipversion 6 
}
proc get_hoststr { {ipversion 6} } {
	if { $ipversion == "6" } {
		set hoststr "::1"
	} elseif { $ipversion == "4" } {
			set hoststr "127.0.0.1"
	} else {
		error "Unrecognized IP version $ipversion, cannot set loopback address."
	}
	return $hoststr
}
...
proc do_sync { myPort clientPorts timeout {verbose 0} } {  
	...
	proc run_server { myHost myPort clients timeout verbose} {
	    set ::numCon 0
		...
		if [catch { socket -server my_connections -myaddr $myHost \
				$myPort } server ] {
		    #if {$verbose} {
		...
		if { $::server_connections == -1 } {
		    puts "Failure, server at port $myPort reached timeout of $timeout seconds before receiving $::numClients connections."
		} else {
	...
	"run_server $hoststr $myPort $numClients $timeout $verbose" \
	::sync_server_results
	...
	set returnVal -1
	set numPorts [llength $clientPorts]
	set myPort [expr {int(rand() * 1000)} + $numPorts + $clientPort]
	if { $verbose } {
	...
	while { $returnVal == -1 && [info exists ::clientconnected] == 0 } {
	    update
	    if [ catch { socket -myport $myPort $hoststr $clientPort} s ] {
		#if {$verbose} {

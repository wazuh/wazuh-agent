 ...
 */
int
__db_win32_mutex_lock(env, mutex, timeout, flags)
	ENV *env;
	db_mutex_t mutex;
 ...
 *
 * PUBLIC: int __db_win32_mutex_readlock __P((ENV *, db_mutex_t, u_int32_t));
 */
int
__db_win32_mutex_readlock(env, mutex, flags)
	ENV *env;
	db_mutex_t mutex;
	u_int32_t flags;
{
 ...
 */
int
__db_win32_mutex_unlock(env, mutex, ip, flags)
	ENV *env;
	DB_THREAD_INFO *ip;
	db_mutex_t mutex;
	u_int32_t flags;

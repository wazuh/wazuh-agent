...
class FileSystemTest : public ::testing::Test
{
    protected:
        ...
        static void SetUpTestSuite()
        {
            ...
            while (!fsWrapper->exists(ROOT_PATH_1) ||
                    !fsWrapper->exists(ROOT_PATH_2) ||
            ...
            while (!fsWrapper->exists(EXPAND_PATH_1) ||
                    !fsWrapper->exists(EXPAND_PATH_2) ||
                    !fsWrapper->exists(EXPAND_PATH_3) ||
                    !fsWrapper->exists(EXPAND_PATH_4))
            {
                if (iteration++ > ITERATION_LIMIT)
                {

        ...
        for (size_t i = 0; i < (handle_bytes_needed / sizeof(HMODULE)); i++) {
            ...
            if (GetModuleFileNameExW(handle_process,
                ...
                if (verify_hash_and_pe_signature(module_name) != OS_SUCCESS) {
                    ...
                    LogCritical(ERROR_MESSAGE, module_name);
#else
                    LogWarn(ERROR_MESSAGE, module_name);
#endif // IMAGE_TRUST_CHECKS == 2
                } else {
                    LogDebug("The file '%S' is signed and its signature is valid.", module_name);
                }
            }
        }
    } else {
...
#if IMAGE_TRUST_CHECKS == 2
        LogCritical(ERROR_MESSAGE, GetLastError());
#else
        LogWarn(ERROR_MESSAGE, GetLastError());
#endif // IMAGE_TRUST_CHECKS == 2
...
#if IMAGE_TRUST_CHECKS == 2
            LogCritical(ERROR_MESSAGE, notification_data->loaded.full_dll_name->Buffer);
#else
            LogWarn(ERROR_MESSAGE, notification_data->loaded.full_dll_name->Buffer);
#endif // IMAGE_TRUST_CHECKS == 2
        } else {
            LogDebug("The file '%S' is signed and its signature is valid.",
                    notification_data->loaded.full_dll_name->Buffer);
        }
#endif // IMAGE_TRUST_CHECKS != 0
        break;
    case LDR_DLL_NOTIFICATION_REASON_UNLOADED:
        LogDebug("Unloaded: '%S'", notification_data->unloaded.full_dll_name->Buffer);
        break;
    }
}
        ...
        if (handle_ntdll) {
            ...
            if (LdrRegisterDllNotification)
            {
                ...
                const char* ERROR_MESSAGE = "The dynamic signature validation is not available for this system. Error"
                                            " %lu: %s";
#if IMAGE_TRUST_CHECKS == 2
                LogCritical(ERROR_MESSAGE, GetLastError(), win_strerror(GetLastError()));
#else
                LogWarn(ERROR_MESSAGE, GetLastError(), win_strerror(GetLastError()));
#endif // IMAGE_TRUST_CHECKS == 2
            }
        } else {

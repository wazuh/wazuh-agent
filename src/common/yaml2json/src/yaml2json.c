...
int yaml_parse_stdin(yaml_document_t * document) {
    ...
    if (yaml_parser_load(&parser, document)) {
        ...
        LogWarn("Failed to load YAML document at line %u", (unsigned int)parser.problem_mark.line);
    }
...
int yaml_parse_file(const char * path, yaml_document_t * document) {
    ...
    if (finput = wfopen(path, "rb"), finput) {
        ...
        if (yaml_parser_load(&parser, document)) {
            ...
            LogWarn("Failed to load YAML document in %s:%u", path, (unsigned int)parser.problem_mark.line);
        ...
        LogWarn("Cannot open file '%s': %s (%d)", path, strerror(errno), errno);
    }
...
cJSON * yaml2json_node(yaml_document_t * document, yaml_node_t * node,int quoted_float_as_string) {
    ...
    case YAML_MAPPING_NODE:
        ...
        for (pair_i = node->data.mapping.pairs.start; pair_i < node->data.mapping.pairs.top; ++pair_i) {
            ...
            if (key->type != YAML_SCALAR_NODE) {
                LogWarn("Mapping key is not scalar (line %u).", (unsigned int)key->start_mark.line);
                continue;
            }
    ...
    default:
        LogWarn("Unknown node type (line %u).", (unsigned int)node->start_mark.line);
        object = NULL;
    }

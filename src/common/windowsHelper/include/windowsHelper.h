...
#ifndef _WINDOWS_HELPER_H
#define _WINDOWS_HELPER_H
...
namespace Utils
{
    ...
    static std::string normalizeTimestamp(const std::string& dateISO8601CalendarDateFormat,
    ...
    class NetworkWindowsHelper final
    {
        static DWORD getAdapterAddresses(PIP_ADAPTER_ADDRESSES& ipAdapterAddresses)
        {
            ...
            // addresses.
            const auto adapterAddressesFlags {isVistaOrLater() ? (GAA_FLAG_INCLUDE_PREFIX | GAA_FLAG_INCLUDE_GATEWAYS)
                                                               : 0};
            ...
            do
            {
                ...
                dwRetVal =
                    GetAdaptersAddresses(AF_UNSPEC, adapterAddressesFlags, nullptr, ipAdapterAddresses, &bufferLen);
...
#pragma warning(pop)
        ...
        static std::string ipv6Netmask(const uint8_t maskLength)
        {
            ...
            if (maskLength < MAX_BITS_LENGTH)
            {
                ...
                static std::map<int, std::string> NET_MASK_FILLER_CHARS_MAP = {{1, "8"}, {2, "c"}, {3, "e"}};

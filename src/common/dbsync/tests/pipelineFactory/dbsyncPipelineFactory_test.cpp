...
void DBSyncPipelineFactoryTest::SetUp()
{
    ...
    m_dbHandle = DBSyncImplementation::instance().initialize(
        HostType::AGENT, DbEngineType::SQLITE3, DATABASE_TEMP, sql, DbManagement::VOLATILE, {});
};
...
TEST_F(DBSyncPipelineFactoryTest, CreatePipelineOk)
{
    ...
    const auto pipeHandle {m_pipelineFactory.create(
        m_dbHandle, json["tables"], threadNumber, maxQueueSize, [](ReturnTypeCallback, const nlohmann::json&) {})};
...
TEST_F(DBSyncPipelineFactoryTest, CreatePipelineInvalidHandle)
{
    ...
    EXPECT_THROW(
        m_pipelineFactory.create(
            handle, json["tables"], threadNumber, maxQueueSize, [](ReturnTypeCallback, const nlohmann::json&) {}),
        DbSync::dbsync_error);
}
...
TEST_F(DBSyncPipelineFactoryTest, CreatePipelineInvalidCallback)
{
    ...
    const unsigned int threadNumber {1};
    const unsigned int maxQueueSize {1000};
    EXPECT_THROW(m_pipelineFactory.create(m_dbHandle, json["tables"], threadNumber, maxQueueSize, nullptr),
                 DbSync::dbsync_error);
}

TEST_F(DBSyncPipelineFactoryTest, GetPipelineInvalidTxnContext)
{
    EXPECT_THROW(m_pipelineFactory.pipeline(nullptr), DbSync::dbsync_error);
}

TEST_F(DBSyncPipelineFactoryTest, PipelineSyncRowInvalidData)
{
    CallbackWrapper wrapper;
    const auto& jsonInputNoTable {
        R"({"data":[{"name":"System","pid":4,"tid":100}],"exception":"[json.exception.out_of_range.403] key 'table' not found"})"};
    const auto& jsonInputNoData {
        R"({"exception":"[json.exception.out_of_range.403] key 'data' not found","table":"processes"})"};
    const auto resultFnc {[&wrapper](ReturnTypeCallback resultType, const nlohmann::json& result)
                          {
    ...
    const int threadNumber {1};
    const int maxQueueSize {1000};
    const auto pipeHandle {m_pipelineFactory.create(m_dbHandle, json["tables"], threadNumber, maxQueueSize, resultFnc)};
    ASSERT_NE(nullptr, pipeHandle);
...
{
    CallbackWrapper wrapper;
    const auto& jsonInputNoTxn {
        R"({"table":"processes","data":[{"pid":4, "tid":100, "name":"System"},{"pid":5, "tid":101, "name":"System1"},{"pid":7, "tid":101, "name":"System7"}]})"};
...
{
    EXPECT_THROW(m_pipelineFactory.destroy(nullptr), DbSync::dbsync_error);
}

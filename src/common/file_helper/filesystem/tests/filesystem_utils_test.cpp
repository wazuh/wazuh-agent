...
protected:
    ...
    void SetUp() override
    {
        ...
        //fsUtils = std::make_unique<filesystem::FileSystemUtils>();
    }
    ...
    void SetupFilesystemWrapperExpectations()
    {
        ...
        .WillRepeatedly(::testing::Invoke([&](const std::string& searchPath)->std::vector<std::filesystem::path>
        {
            ...
            // convert to paths and remove all extra path components but one past the ones present in searchPath
...
TEST_F(FileSystemTest, FilesystemExpandWildcardWithPrefixFull)
{
    ...
    fsUtils->expand_absolute_path(PATH_TO_EXPAND_7, output);

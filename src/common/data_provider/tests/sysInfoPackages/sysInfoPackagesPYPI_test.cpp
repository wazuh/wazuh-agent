...
TEST_F(PYPITest, getPackagesTest)
{
    ...
    EXPECT_CALL(*rawFileSystemWrapper, list_directory(_)).WillRepeatedly(Return(fakeFiles));
    EXPECT_CALL(*rawMockedFileIOUtils, readLineByLine(_, _)).WillRepeatedly(Return());
...
{
    EXPECT_CALL(*rawFileSystemWrapper, exists(_)).WillRepeatedly(Return(true));
    EXPECT_CALL(*rawFileSystemWrapper, is_directory(_)).WillRepeatedly(Return(false));
...
TEST_F(PYPITest, getPackages_OneValidPackageTestEggInfo)
{
    ...
    EXPECT_CALL(*rawFileSystemWrapper, is_directory(_)).WillRepeatedly(Return(true));
    EXPECT_CALL(*rawFileSystemWrapper, list_directory(_)).WillRepeatedly(Return(fakeFiles));
    EXPECT_CALL(*rawFileSystemWrapper, is_regular_file(_)).WillRepeatedly(Return(true));

    std::vector<std::string> fakePackageLines = {"Name: TestPackage", "Version: 1.0.0"};
    EXPECT_CALL(*rawMockedFileIOUtils, readLineByLine(std::filesystem::path("/fake/dir/egg-info"), _))
        .WillOnce(
...
TEST_F(PYPITest, getPackages_OneValidPackageTestNoRegularFileDistInfo)
{
    ...
    EXPECT_CALL(*rawFileSystemWrapper, is_directory(_)).WillRepeatedly(Return(true));
    EXPECT_CALL(*rawFileSystemWrapper, list_directory(_)).WillRepeatedly(Return(fakeFiles));
    EXPECT_CALL(*rawFileSystemWrapper, is_regular_file(_)).WillRepeatedly(Return(false));

    std::vector<std::string> fakePackageLines = {"Name: TestPackage", "Version: 1.0.0"};
    EXPECT_CALL(*rawMockedFileIOUtils, readLineByLine(std::filesystem::path("/fake/dir/dist-info/METADATA"), _))
        .WillOnce(
            [&](const std::filesystem::path&, const std::function<bool(const std::string&)>& callback)
            {
                for (const auto& line : fakePackageLines)
...
TEST_F(PYPITest, getPackages_OneValidPackageTestDistInfo)
{
    ...
    std::vector<std::string> fakePackageLines = {"Name: TestPackage", "Version: 1.0.0"};
    EXPECT_CALL(*rawMockedFileIOUtils, readLineByLine(std::filesystem::path("/fake/dir/dist-info"), _))
        .WillOnce(
            [&](const std::filesystem::path&, const std::function<bool(const std::string&)>& callback)
            {
                for (const auto& line : fakePackageLines)
...
TEST_F(PYPITest, getPackages_OneValidPackageTestNoRegularFileEggInfo)
{
    ...
    std::vector<std::string> fakePackageLines = {"Name: TestPackage", "Version: 1.0.0"};
    EXPECT_CALL(*rawMockedFileIOUtils, readLineByLine(std::filesystem::path("/fake/dir/egg-info/PKG-INFO"), _))
        .WillOnce(
            [&](const std::filesystem::path&, const std::function<bool(const std::string&)>& callback)
            {
                for (const auto& line : fakePackageLines)
...
TEST_F(PYPITest, getPackages_MultipleValidPackagesTest)
{
    ...
    EXPECT_CALL(*rawMockedFileIOUtils, readLineByLine(std::filesystem::path("/fake/dir1/egg-info"), _))
        .WillOnce(
            [&](const std::filesystem::path&, const std::function<bool(const std::string&)>& callback)
            {
    ...
    EXPECT_CALL(*rawMockedFileIOUtils, readLineByLine(std::filesystem::path("/fake/dir2/dist-info"), _))
        .WillOnce(
            [&](const std::filesystem::path&, const std::function<bool(const std::string&)>& callback)
            {
